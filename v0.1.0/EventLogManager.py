...
class EventLogManager:
    ...
    def _parse_journald_entry(self, j, raw_line):
        """
        Convert a single parsed journald JSON object into a normalised record.
        Exposed so LiveRunner can reuse parsing without going through the full
        ...
        pid      = int(pid_raw) if pid_raw and str(pid_raw).isdigit() else None
        hostname = j.get('_HOSTNAME', platform.node())
        facility = self._syslog_facility_name(j.get('SYSLOG_FACILITY', ''))

        return self._make_record(
            os_type='Linux', os_source='journald',
            timestamp=ts, source=source, level=level,
            event_id=None, pid=pid, hostname=hostname,
            facility=facility, message=msg.strip(), raw=raw_line,
        )

    def _parse_win_event(self, evt, channel):
        """
        Convert a single win32evtlog event object into a normalised record.
        Exposed so LiveRunner can reuse parsing without going through the full
        batch collection method.
        """
        try:
            ...
            msg = ' | '.join(evt.StringInserts or [])
        try:
            ts = datetime.fromtimestamp(int(evt.TimeGenerated)).isoformat()
        except Exception:
            ts = str(evt.TimeGenerated)
        level    = _WIN_LEVEL.get(evt.EventType, 'UNKNOWN')
        event_id = evt.EventID & 0xFFFF
        return self._make_record(
            os_type='Windows', os_source=channel,
            timestamp=ts, source=str(evt.SourceName), level=level,
            event_id=event_id, pid=None, hostname=str(evt.ComputerName),
            facility=None, message=msg.strip(),
            raw='EventID={} Src={} Inserts={}'.format(
                event_id, evt.SourceName, evt.StringInserts),
        )
